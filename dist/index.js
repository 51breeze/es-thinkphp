...
function createCache() {
  ...
  function set(key2, name, value) {
    let dataset2 = records2.get(key2);
    if (!dataset2) {
      records2.set(key2, dataset2 = /* @__PURE__ */ new Map());
    }
  ...
  function clear(key2) {
    if (import_Utils2.default.isCompilation(key2)) {
      key2.modules.forEach((module2) => {
        records2.delete(module2);
      });
      records2.forEach((v, stack) => {
        if (stack && stack.compilation === key2) {
  ...
  "node_modules/@easescript/transform/lib/core/DependFile.js"() {
    ...
    DependFile = class {
      static create(dir, files = []) {
        ...
        let key2 = String(dir).toLowerCase();
        let instance = dataset.get(key2);
        if (!instance) {
          instance = new DependFile(dir);
          dataset.set(key2, instance);
        }
...
function createRouteInstance(ctx, module2, owner, path12, method, meta = null, params = [], defaultValue = {}, isRouterModule = false, isWebComponent = false) {
  let action = null;
  ...
  let pathName = path12 ? String(path12).trim() : action;
...
function getAnnotationArgument(name, args, indexes = null) {
  ...
  let index = args.findIndex((item) => {
    ...
    if (key2 === name)
      return true;
    if (pluralArgumentNames[name] === key2)
      return true;
...
function createHttpAnnotationNode(ctx, stack) {
  ...
  if (!route) {
    let path12 = result.module.getName() + ":" + result.method.value();
    stack.error(10102, path12);
    return null;
  }
...
function createRouteConfigNodeForHttpRequest(ctx, route, paramArg = null) {
  if (!route)
    return null;
  let path12 = route.path;
  let defaultParams = [];
  let allowMethodNode = ctx.createArrayExpression(
    route.method.split(",").map((val) => ctx.createLiteral(val.trim()))
  );
  Object.keys(route.defaultValue).forEach((key2) => {
    defaultParams.push(ctx.createProperty(
      ctx.createIdentifier(key2),
      Node_default.is(route.defaultValue[key2]) ? route.defaultValue[key2] : ctx.createChunkExpression(route.defaultValue[key2], false)
    ));
  });
  if (route.params.length > 0) {
    path12 = [path12, ...route.params.map((item) => {
      let name = item.name;
      if (item.optional) {
        name += "?";
      }
...
var import_Utils6, import_source_map, disabledNewLine, Generator2, Generator_default;
...
var KIND_CLASS, KIND_INTERFACE, KIND_ENUM, KIND_STRUCT, KIND_VAR, KIND_CONST, KIND_METHOD, KIND_ACCESSOR, KIND_ENUM_PROPERTY, KIND_STRUCT_COLUMN, MODIFIER_STATIC, MODIFIER_PUBLIC, MODIFIER_PROTECTED, MODIFIER_PRIVATE, MODIFIER_ABSTRACT, MODIFIER_FINAL, MODIFIER_OPTIONAL, PRIVATE_NAME;
var init_Constant = __esm({
  "node_modules/@easescript/transform/lib/core/Constant.js"() {
    ...
    KIND_METHOD = 1 << 6;
    KIND_ACCESSOR = 1 << 7;
    KIND_ENUM_PROPERTY = 1 << 8;
    KIND_STRUCT_COLUMN = 1 << 9;
    MODIFIER_STATIC = 1 << 10;
    MODIFIER_PUBLIC = 1 << 11;
    MODIFIER_PROTECTED = 1 << 12;
    MODIFIER_PRIVATE = 1 << 13;
    MODIFIER_ABSTRACT = 1 << 14;
    MODIFIER_FINAL = 1 << 15;
    MODIFIER_OPTIONAL = 1 << 16;
    ...
    let result = annotations.map((annot) => {
      ...
      if (name === "embed") {
        ...
        let _args = args.map((arg, index) => {
          ...
          if (route && (key2 === "path" || !arg.assigned && index === 0)) {
            return;
          }
          if (!key2)
            key2 = index;
...
var import_Generator5, VirtualModule2;
var init_VirtualModule2 = __esm({
  "node_modules/@easescript/es-php/lib/core/VirtualModule.js"() {
    init_VirtualModule();
    init_Common2();
    import_Generator5 = __toESM(require_Generator());
...
// lib/vms/Routes.js
var import_Utils49, import_path11, key, Routes, Routes_default;
var init_Routes = __esm({
  "lib/vms/Routes.js"() {
    init_VirtualModule2();
    import_Utils49 = __toESM(require("easescript/lib/core/Utils"));
    ...
    Routes = class extends VirtualModule2 {
      ...
      async build(ctx, graph) {
        ...
        if (outfile == null) {
          ...
          outfile = import_Utils49.default.normalizePath(import_path11.default.join(import_path11.default.dirname(outfile), filename + (ctx.options.outExt || ".php")));
        }
...
var require_ClassBuilder = __commonJS({
  "lib/core/ClassBuilder.js"(exports, module2) {
    init_ClassBuilder();
    init_Common();
    var import_Utils50 = __toESM(require("easescript/lib/core/Utils"));
    init_Routes();
    var ClassBuilder4 = class extends ClassBuilder_default2 {
      parseMethodRoute(ctx, stack) {
        if (!stack.isMethodDefinition || stack.isAccessor || stack.isConstructor || !import_Utils50.default.isModifierPublic(stack)) {
          return;
        }
        ...
        if (routes && routes.length > 0) {
          routes.forEach((route) => {
            ...
            let { method, action, path: path12, params } = route;
            vm.append(ctx, stack, {
              className,
              method,
              action,
...
var Token = class {
  ...
  createFunctionDeclaration(key2, block, params = []) {
    ...
    node.key = Node_default.is(key2) ? key2 : this.createIdentifier(key2);
    return node;
  }
...
var TableBuilder = class {
  ...
  createTable(ctx, stack) {
    ...
    const key2 = module2.getName();
    if (this.hasTable(key2))
      return false;
    ...
    const make = (stack2) => {
      ...
      stack2.body.forEach((item) => {
        ...
        if (item.isStructTableColumnDefinition) {
          const key3 = item.key.value();
          if (cacheColumn[key3] === true) {
            return;
          }
          cacheColumn[key3] = true;
        }
        if (token) {
          if (item.isStructTableColumnDefinition) {
          ...
          if (item.key && item.key.isIdentifier) {
            const key3 = String(item.raw()).replace(/\s\t\r\n/g, "").toLowerCase();
            if (cacheOption[key3] === true) {
              return;
            }
            cacheOption[key3] = true;
          }
    ...
    gen.make(node);
    this.#records.set(key2, gen.toString());
    this.#changed = true;
    this.build(ctx);
    return true;
  }
...
var ClassBuilder = class {
  ...
  createMemberDecorator(ctx, decorators, key2, descriptor) {
...
function createElementKeyPropertyNode(ctx, stack) {
  ...
  if (fills || all) {
    ...
    if (all || isCondition || fills.includes(direName)) {
      return createElementKeyNode(ctx, stack, isForContext ? ctx.createIdentifier(key2 || "key") : null);
    }
  }
}
    ...
    if (item.isIdentifier || item.isMemberExpression) {
      ...
      const make = (type3) => {
        ...
        if (type3.isIntersectionType) {
          ...
          Array.from(type3.descriptors.keys()).forEach((key2) => {
            const items = type3.descriptors.get(key2);
...
var globalModules = ["Array", "Object", "Boolean", "Math", "Number", "String", "Error"];
var Context2 = class extends Context_default {
  ...
  createModuleUsing(depModule, context) {
    if (!globalModules.includes(depModule.id)) {
      const defines = getModuleAnnotations(depModule, ["define"]);
      if (defines.length > 0) {
        const has = defines.some((define) => {
          const data = parseDefineAnnotation(define);
          if (data.use != null && data.use !== false) {
            let local = this.getModuleAlias(depModule, context);
            let source2 = data.use;
            if (source2 === "") {
              source2 = "\\" + depModule.id;
            } else if (source2.includes(".")) {
              source2 = source2.split(".").join("\\");
            } else if (source2.includes("/")) {
              source2 = source2.split("/").join("\\");
            }
            this.addUsing(source2, local);
            return true;
          }
        });
        if (has) {
  ...
  getModuleReferenceName(module2, context = null) {
    ...
    if (inNs && this.isGlobalModule(module2) && !this.isVModule(module2)) {
      return "\\" + name;
    }
    return name;
  }
  ...
  isGlobalModule(module2) {
    return module2.isDeclaratorModule && module2.getStacks().some((stack) => {
      const compilation = stack.compilation;
      if (compilation && compilation.isGlobalFlag) {
        const scopes = compilation.pluginScopes;
        if (scopes) {
  ...
  getModuleMappingFolder(module2) {
    ...
    if (isRM && this.isGlobalModule(module2)) {
      file += ".global";
  ...
  getModuleNamespace(module2, suffix = null) {
    ...
    if (folder) {
      folder = folder.replace(/(^|\\)(interface|class)(\\|$)/, "$1_$2$3");
      if (suffix) {
    ...
    if (module2.namespace && module2.namespace.isNamespace) {
      let items = module2.namespace.getChain();
      if (items.length > 0) {
        items = items.map((item) => item === "interface" ? "_interface" : item);
        if (suffix) {
  ...
  createNamespaceStatement(source) {
    ...
    source = source.replace(/(^|\\)(interface|class)(\\|$)/, "$1_$2$3");
    node.source = source;
    return node;
  }
...
function FunctionExpression_default2(ctx, stack, type) {
  ...
  if (stack.expression && stack.expression.async || stack.async) {
    ...
    if (!block || block.type !== "BlockStatement") {
      block = ctx.createBlockStatement(block ? [block] : []);
    }
...
// node_modules/@easescript/es-php/lib/transforms/Boolean.js
init_Object();
init_Common2();
...
function createNode2(ctx, stack) {
  ...
  if (desc2 && !isReflect && stack.right) {
    if (operator2 && operator2.charCodeAt(0) === 43) {
      ...
      const isNumber = leftType === "Number" && leftType === rightType;
      if (!isNumber) {
        node.operator = operator2.length > 1 ? "." + operator2.substr(1) : ".";
      }
    }
...
function BinaryExpression_default2(ctx, stack) {
  ...
  if (operator2.charCodeAt(0) === 43) {
    ...
    leftType = ctx.getAvailableOriginType(leftType);
    rightType = ctx.getAvailableOriginType(rightType);
    let isNumber = leftType === "Number" && leftType === rightType;
    if (!isNumber) {
...
function Identifier_default2(ctx, stack) {
  ...
  let isConst = desc2 && desc2.isDeclaratorVariable && desc2.kind === "const";
  if (!isConst && isDeclarator) {
  ...
  if (!isConst) {
    if (desc2 && (desc2.isFunctionDeclaration || desc2.isDeclaratorVariable)) {
      isDeclarator = true;
    } else if (stack.parentStack.isMemberExpression) {
      isDeclarator = false;
...
function createChildNode2(ctx, stack, childNode, prev = null) {
  ...
  while (directives.length > 0) {
    ...
    if (name === "each" || name === "for") {
      ...
      let key2 = ctx.createVarIdentifier(valueArgument.declare.key || "key");
      let index = valueArgument.declare.index;
      if (index) {
        index = ctx.createIdentifier(index);
      }
...
function createDirectiveElementNode2(ctx, stack, children) {
  ...
  switch (name) {
    ...
    case "each": {
      ...
      let key2 = argument.key || ctx.createVarIdentifier("key");
...
function Literal_default2(ctx, stack) {
  ...
  if (type) {
    ...
    if (value && value.toLowerCase() === "regexp") {
...
// node_modules/@easescript/es-php/lib/core/StructTableBuilder.js
...
function addResolveRule(glob2, resolve) {
  Object.keys(resolve.namespaces).forEach((key2) => {
    glob2.addRuleGroup(key2, resolve.namespaces[key2], "namespaces");
  });
  Object.keys(resolve.folders).forEach((key2) => {
    glob2.addRuleGroup(key2, resolve.folders[key2], "folders");
  });
      ...
      if (typeof key2 === "object") {
        if (key2.test === void 0 || key2.value === void 0) {
          ...
          if (typeof key2.value === "function") {
            glob2.addRuleGroup(key2.test, key2.value, "usings");
          } else {
            glob2.addRuleGroup(key2.test, () => key2.value, "usings");
          }
        }
      } else {
        glob2.addRuleGroup(key2, trueCallback, "usings");
      }
    });
  } else {
    Object.keys(resolve.usings).forEach((key2) => {
      if (typeof resolve.usings[key2] === "function") {
        glob2.addRuleGroup(key2, resolve.usings[key2], "usings");
      } else {
...
var Plugin2 = class extends Plugin {
  ...
  getWidget(name) {
    ...
    if (name === "token") {
      ...
      Object.keys(tokens_exports).forEach((key2) => {
        if (!_tokens[key2]) {
          _tokens[key2] = tokens_exports[key2];
        }
      });
...
// lib/core/Context.js
var Context3 = class extends Context_default2 {
  isPermissibleRouteProvider(moduleOrMethodStack) {
    return false;
  }
};
var Context_default3 = Context3;
